        ...
        html += `
            <div class="rental-card">
                <div class="rental-image">
                    <img src="${car.image}" alt="${car.title}">
                    <div class="rental-badge">${car.type.charAt(0).toUpperCase() + car.type.slice(1)}</div>
                </div>
                <div class="rental-details">
                    <h3 class="rental-title">${car.title}</h3>
                    <div class="rental-info">
                        <div class="rental-price">$${car.price} <span>/ day</span></div>
                        <div class="car-year">${car.brand.charAt(0).toUpperCase() + car.brand.slice(1)}</div>
                    </div>
        ...
        html += `
            <div class="part-card">
                <div class="part-image">
                    ...
                    <div class="part-badge">${part.condition.charAt(0).toUpperCase() + part.condition.slice(1)}</div>
                </div>
                <div class="part-details">
                    <h3 class="part-title">${part.title}</h3>
                    <div class="part-info">
                        <div class="part-price">$${part.price}</div>
                        <div class="part-brand">${part.brand.charAt(0).toUpperCase() + part.brand.slice(1)}</div>
                    </div>
                    <div class="part-compatibility">
                        <i class="fas fa-car"></i> Compatible with: ${part.compatibility}
                    </div>
...
function populateDealers(filteredDealers = dealers) {
    ...
    filteredDealers.forEach(dealer => {
        ...
        dealer.brands.forEach(brand => {
            brandBadges += `<span class="dealer-brand">${brand.charAt(0).toUpperCase() + brand.slice(1)}</span>`;
        });
...
function filterCarListings() {
    const typeFilters = Array.from(document.querySelectorAll('.car-filter[data-filter="type"]:checked')).map(el => el.value);
    const brandFilters = Array.from(document.querySelectorAll('.car-filter[data-filter="brand"]:checked')).map(el => el.value);
    const fuelFilters = Array.from(document.querySelectorAll('.car-filter[data-filter="fuel"]:checked')).map(el => el.value);
    const conditionFilters = Array.from(document.querySelectorAll('.car-filter[data-filter="condition"]:checked')).map(el => el.value);
    const priceRange = document.getElementById('price-range');
    ...
    let filteredCars = carListings.filter(car => {
        ...
        // Filter by type
        if (typeFilters.length > 0 && !typeFilters.includes(car.type.toLowerCase())) return false;
        
        // Filter by brand
        if (brandFilters.length > 0 && !brandFilters.includes(car.brand.toLowerCase())) return false;
        
        // Filter by fuel type
        if (fuelFilters.length > 0 && !fuelFilters.includes(car.fuelType.toLowerCase())) return false;
...
function filterRentalCars() {
    const typeFilters = Array.from(document.querySelectorAll('.rental-filter[data-filter="type"]:checked')).map(el => el.value);
    const brandFilters = Array.from(document.querySelectorAll('.rental-filter[data-filter="brand"]:checked')).map(el => el.value);
    const featureFilters = Array.from(document.querySelectorAll('.rental-filter[data-filter="features"]:checked')).map(el => el.value);
    const priceRange = document.getElementById('price-range');
    ...
    let filteredCars = rentalCars.filter(car => {
        ...
        if (featureFilters.length > 0) {
            const hasAllFeatures = featureFilters.every(feature => car.features.includes(feature));
...
function filterCarParts() {
    const categoryFilters = Array.from(document.querySelectorAll('.parts-filter[data-filter="category"]:checked')).map(el => el.value);
    const brandFilters = Array.from(document.querySelectorAll('.parts-filter[data-filter="brand"]:checked')).map(el => el.value);
    const conditionFilters = Array.from(document.querySelectorAll('.parts-filter[data-filter="condition"]:checked')).map(el => el.value);
    ...
    let filteredParts = carParts.filter(part => {
        ...
        // Filter by category
        if (categoryFilters.length > 0 && !categoryFilters.includes(part.category)) return false;
        
        // Filter by brand
        if (brandFilters.length > 0 && !brandFilters.includes(part.brand)) return false;
        
        // Filter by condition
        if (conditionFilters.length > 0 && !conditionFilters.includes(part.condition)) return false;
...
function filterDealers() {
    const typeFilters = Array.from(document.querySelectorAll('.dealer-filter[data-filter="type"]:checked')).map(el => el.value);
    const brandFilters = Array.from(document.querySelectorAll('.dealer-filter[data-filter="brand"]:checked')).map(el => el.value);
    const serviceFilters = Array.from(document.querySelectorAll('.dealer-filter[data-filter="services"]:checked')).map(el => el.value);
    const ratingFilters = Array.from(document.querySelectorAll('.dealer-filter[data-filter="rating"]:checked')).map(el => parseInt(el.value));
    
    let filteredDealers = dealers.filter(dealer => {
        // Filter by type
        if (typeFilters.length > 0 && !typeFilters.some(type => dealer.type.includes(type))) return false;
        
        // Filter by brand
        if (brandFilters.length > 0 && !brandFilters.some(brand => dealer.brands.includes(brand))) return false;
        
        // Filter by services
        if (serviceFilters.length > 0 && !serviceFilters.every(service => dealer.services.includes(service))) return false;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (resetRentalFilters) {
        resetRentalFilters.addEventListener('click', () => {
            ...
            if (priceRange) {
                ...
                document.getElementById('price-value').textContent = '$250';
            }
    ...
    if (marketplacePriceRange) {
        marketplacePriceRange.addEventListener('input', () => {
            document.getElementById('price-value').textContent = '$' + marketplacePriceRange.value;
        });
    }
    ...
    if (partsPriceRange) {
        partsPriceRange.addEventListener('input', () => {
            document.getElementById('parts-price-value').textContent = '$' + partsPriceRange.value;
        });
    }
    ...
    categoryCards.forEach(card => {
        card.addEventListener('click', () => {
            ...
            if (category) {
                ...
                const categoryCheckbox = document.querySelector(`.parts-filter[data-filter="category"][value="${category}"]`);
                if (categoryCheckbox) {
